...
const Header = (props) => {
  const { current, title = "DBC Badminton Court" } = props;
  const [cookies, setCookie, removeCookie] = useCookies(["currentuser"]);
  const navigate = useNavigate();
  const { currentuser } = cookies;
  const role = cookies.currentuser?.role;
  return (
    <Box
      sx={{
        py: 4,
    ...
    >
      ...
      {currentuser && (
        <Typography variant="body1" align="center">
          Current Logged In {currentuser.role}:
          <br />
      ...
      >
        ...
        {role === "admin" && (
          <Button
            component={Link}
            to="/dashboard"
            variant={current === "dashboard" ? "contained" : "outlined"}
          >
            Dashboard
          </Button>
        )}
